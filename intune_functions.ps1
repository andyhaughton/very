...
$Max = '?$top=999'
    ...
    $uri = "https://graph.microsoft.com/$graphApiVersion/$($Resource)$($Max)"
    (Invoke-RestMethod -Uri $uri –Headers $authToken –Method Get).value
...
$graphApiVersion = "Beta"
$Resource = "devices/$($device)/memberof"
...
$graphApiVersion = "Beta"
$Resource = "deviceAppManagement/mobileApps/$ApplicationId/deviceStatuses"
...
#$User = Read-Host -Prompt "Enter the email address  of the user to check application assignment"
 #$device = Read-Host -Prompt "Enter the name of the user's device to check application assignment"
